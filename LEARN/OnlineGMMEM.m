function [GMM,llh] = OnlineGMMEM(GMM0,DataSource,NumIterations,MiniBatchSize,OutputFile,T0,alpha,FirstBatchSize,removeFlatPatches)
  ...
  if isstruct(GMM0)
    ...
    K = GMM0;
    GMM.dim = size(DataSource(1),1);
    GMM.nmodels = K;
    GMM.mixweights = zeros(1,K);
    GMM.means = zeros(size(DataSource(1),1),K);
...
%    end
  ...
  for t=1:NumIterations
    ...
    plot(sort(GMM.mixweights,'descend'));
    set(gca,'YLim',[0 1]);
    ...
    if mod(t,10)==0
      save("-7",OutputFile,'GMM','t','NumIterations','eta','MiniBatchSize','llh','alpha','T0');
    end
  end
